...
XRESULT D3D11PFX_HDR::Render( RenderToTextureBuffer* fxbuffer ) {
	...
	// Draw the HDR-Shader
	auto hps = engine->GetShaderManager().GetPShader( L"PS_PFX_HDR" );
	hps->Apply();
...
void D3D11PFX_HDR::CreateBloom( RenderToTextureBuffer* lum ) {
	...
	auto tonemapPS = engine->GetShaderManager().GetPShader( L"PS_PFX_Tonemap" );
	tonemapPS->Apply();
	...
	auto gaussPS = engine->GetShaderManager().GetPShader( L"PS_PFX_GaussBlur" );


	/** Pass 1: Blur-H */
	// Apply PFX-VS
	auto simplePS = engine->GetShaderManager().GetPShader( L"PS_PFX_Simple" );
...
RenderToTextureBuffer* D3D11PFX_HDR::CalcLuminance() {
	...
	auto lps = engine->GetShaderManager().GetPShader( L"PS_PFX_LumConvert" );
	lps->Apply();
	...
	auto aps = engine->GetShaderManager().GetPShader( L"PS_PFX_LumAdapt" );
	aps->Apply();
